...
export function EventReviews({ eventId, eventTitle, showTitle = true }: Props) {
  ...
  useEffect(() => {
    async function loadReviews() {
      ...
      const { data, error } = await supabase
        .from('event_reviews')
        .select('*, profiles(full_name, business_name, avatar_url, city)')
        .eq('event_id', eventId)
      ...
      setReviews((data || []) as EventReview[])
      setLoading(false)
    }
    ...
    <div className="space-y-6">
      {showTitle && (
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-bold text-slate-800 text-lg">Rating & Komentar Event</h3>
            {eventTitle && <p className="text-xs text-slate-500 mt-0.5">{eventTitle}</p>}
          </div>
      ...
      {totalReviews === 0 ? (
        <Card className="p-8">
          <Empty
            ...
            sub={fetchError ? `Gagal memuat data: ${fetchError}` : "Event ini belum memiliki ulasan dari peserta UMKM."}
          />
        </Card>
      ) : (
        <div className="space-y-4">
          {/* Summary Breakdown Card */}
          <Card className="p-5 bg-gradient-to-br from-slate-900 to-indigo-950 text-white border-0 shadow-xl">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div>
                          <h4 className="font-bold text-slate-800 text-sm">
                            {profile?.business_name || profile?.full_name || 'UMKM Peserta'}
                          </h4>
                          {profile?.city && (
